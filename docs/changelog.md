...
## 变更 #34：README 删除「部署到公网」章节（2026-08-26）

- **原因**：该章节包含 Railway/Render/ngrok 等公网部署指引，用户认为不适合展示给项目访客（此前已确认暂不做公网部署），删除以免误导。
- **修改位置**：`README.md` 删除 `## 部署到公网（让别人直接访问）` 整节（含 PaaS / 云服务器 / 内网穿透三种方式及境外数据源限流提示），Docker 部署与「参与贡献」章节衔接不变。
- **测试方式**：`rg -n "部署到公网|PaaS|ngrok" README.md` 无结果；人工核对相邻章节格式（无多余空行）。
- **可能影响的模块**：仅 README 文档；不影响代码、jar、Release 资产（本次不发版）。
