...
或直接 `java -jar target/dcf-valuation-tool-1.1.5.jar --server.port=8502`）。
...
- **免费 PaaS（最省事）**：把本仓库部署到 Railway / Render / Fly.io（仓库已内置 Dockerfile），
  构建后即可获得公网地址，适合演示与试用
- **云服务器**：在服务器（需 JDK 21）上运行 `java -jar target/dcf-valuation-tool-1.1.5.jar`，
